...
from app.api.deps import get_optional_user, get_verified_user
...
async def create_post(
    ...
    post = await PostService(db).create_post(author=current_user, content=payload.content)
...
async def get_post(
    ...
    post = await PostService(db).get_post(post_id, viewer_id=viewer.id if viewer else None)
    return PostOut.model_validate(post)
